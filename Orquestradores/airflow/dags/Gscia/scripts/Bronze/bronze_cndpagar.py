...
PREFIX = os.getenv('MINIO_PREFIX', 'ahreas/cndpagar')
...
# Diretórios de saída
# =========================
BASE_DIR = os.getenv('BASE_DIR', '/opt/airflow/data/Gscia/ahreas/cndpagar')
...
def upload_to_minio(latest_parquet: str, history_parquet: str, timestamp: str) -> None:
    ...
    try:
        ...
        latest_path = f"{MINIO_ALIAS}/{BUCKET}/{PREFIX}/latest/cndpagar.parquet"
        subprocess.run(
        ...
        # Upload do histórico
        history_path = f"{MINIO_ALIAS}/{BUCKET}/{PREFIX}/history/cndpagar_{timestamp}.parquet"
        subprocess.run(
...
def extract_cndpagar_data():
    """Extrai dados da tabela cndpagar do SQL Server"""
    print("=" * 80)
    print("🚀 Iniciando extração da camada RAW - Tabela: cndpagar")
    print("=" * 80)
    ...
    query = "SELECT * FROM [dbo].[cndpagar]"
...
def save_and_upload_data(df: pd.DataFrame):
    ...
    latest_parquet = os.path.join(LATEST_DIR, "cndpagar.parquet")
    history_parquet = os.path.join(HISTORY_DIR, f"cndpagar_{timestamp}.parquet")
...
def cleanup_old_files(max_files: int = MAX_HISTORY_FILES):
    ...
    files = sorted(
        glob.glob(os.path.join(HISTORY_DIR, "cndpagar_*.parquet")),

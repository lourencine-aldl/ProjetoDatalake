...
ENDPOINT = "/api/vehicles/v2/list"  # Power Query: "api/vehicles/v2/list?"
PAGE_SIZE = 500
MAX_PAGES = 1000

# destino no MinIO
MINIO_S3_PATH = "s3://datalake-sost/api/zuq/zuq_vehicles/zuq_vehicles.parquet"

# ExpandRecordColumn(Fonte, "row", {...})
ROW_FIELDS = ["id", "code", "licensePlate", "vehicleStatus", "deviceCode"]
...
# LOGGING
# =========================
logger = logging.getLogger("zuq_vehicles")
...
def zuq_get_paged(
    ...
    # Power Query passou (1, 500) => page=1, pageSize=500
...
# =========================
def expand_and_rename(raw_records: List[Dict[str, Any]]) -> pd.DataFrame:
    """
    - ExpandRecordColumn(Fonte, "row", ROW_FIELDS)
    - RenameColumns conforme RENAME_MAP
    """
    if not raw_records:
        return pd.DataFrame()

    # No Power Query existe uma coluna "row" (record). Aqui aceitamos ambos:
    # - registros com {"row": {...}}
    # - registros já no root
    ...
    for rec in raw_records:
        if isinstance(rec, dict) and isinstance(rec.get("row"), dict):
            ...
            row_dict = rec
        rows.append(row_dict if isinstance(row_dict, dict) else {})
    ...
    # Renomear colunas como no Power Query
...
def main() -> None:
    logger.info("Iniciando extração ZUQ Vehicles: %s%s", BASE_URL, ENDPOINT)
    ...
    # Se quiser forçar o bucket do .env:
    # s3_path = f"s3://{minio_cfg['bucket']}/api/zuq/zuq_vehicles/zuq_vehicles.parquet"

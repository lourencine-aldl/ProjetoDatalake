#!/usr/bin/env python3
"""
Bronze Layer ETL: Extração TAB_FATURAMENTO_FULL PostgreSQL → MinIO (Polars)

Ajustes:
- Colunas obtidas via SELECT ... LIMIT 0 (mais confiável que information_schema)
- Normalização Dremio-friendly ANTES de virar Polars:
  - datetime -> datetime64[ms] (evita TIMESTAMP(NANOS) no Parquet)
  - Decimal/numeric em object -> float64 (bronze)
  - inteiros “vazios” -> Int64 nullable
- Mantém cursor server-side (named cursor)
"""
...
class TableSpec:
    ...
    itersize: int = 10_000  # buffer do server-side cursor
...
def minio_fs(cfg: MinioConfig) -> s3fs.S3FileSystem:
    return s3fs.S3FileSystem(
        ...
        default_fill_cache=False,  # ajuda a poupar RAM
    )
...
def get_columns(conn, sql: str) -> list[str]:
    """Obtém colunas com SELECT ... LIMIT 0 (mais fiel e ordenado)."""
    with conn.cursor() as cur:
...
# ===========================
# NORMALIZAÇÃO DREMIO (via Pandas antes de virar Polars)
# ===========================
def normalize_for_dremio_pd(df: pd.DataFrame) -> pd.DataFrame:
    """
    Estratégia robusta:
    - qualquer datetime -> datetime64[ms] (evita TIMESTAMP(NANOS))
    - object com Decimal/número -> float64
    - object int-ish -> Int64 nullable
    """
    out = df.copy()
    ...
    for c in out.columns:
        ...
        if pd.api.types.is_datetime64_any_dtype(s):
            ...
            if not sample.empty and any(hasattr(v, "year") and hasattr(v, "month") and hasattr(v, "day") for v in sample):
                out[c] = pd.to_datetime(out[c], errors="coerce").astype("datetime64[ms]")

    # 2) object numérico/Decimal -> float64
    for c in out.columns:
        s = out[c]
        if s.dtype != "object":
            continue
        ...
        # detecta Decimal sem importar decimal.Decimal (evita overhead)
        def _is_decimal(v) -> bool:
            return str(type(v)).endswith("Decimal'>")

        if all(isinstance(v, (int, float)) or _is_decimal(v) for v in sample):
            out[c] = pd.to_numeric(out[c], errors="coerce").astype("float64")

    # 3) object int-ish -> Int64
    for c in out.columns:
        s = out[c]
        if s.dtype != "object":
            continue
...
def make_s3_path(bucket: str, schema: str, table: str, ingestion_dt: datetime, part: int) -> str:
    return f"s3://{bucket}/bronze/006_bronze_tab_faturamento_full/part-{part + 1:05d}.parquet"
...
def extract_to_minio(conn, fs: s3fs.S3FileSystem, spec: TableSpec, minio_cfg: MinioConfig, ingestion_dt: datetime) -> dict:
    ...
    # ✅ Colunas via LIMIT 0 (mais confiável que information_schema)
    ...
    try:
        while True:
            ...
            # Pandas -> normaliza -> Polars
            df_pd = pd.DataFrame.from_records(rows, columns=cols)
            df_pd = df_pd.infer_objects(copy=False)
            df_pd = normalize_for_dremio_pd(df_pd)
...
def main():
    ...
    spec = TableSpec(schema="public", name="tab_faturamento_full", chunk_rows=50_000, itersize=10_000)
    ...
    s3_path = f"s3://{minio_cfg.bucket}/bronze/006_bronze_tab_faturamento_full/part-00001.parquet"

#!/usr/bin/env python3
"""
Bronze Layer ETL: Extração TAB_CLIENTE PostgreSQL → MinIO (Polars)
"""
...
logger = logging.getLogger("bronze_003_tab_cliente")
...
def make_s3_path(bucket: str, schema: str, table: str, ingestion_dt: datetime, part: int) -> str:
    return f"s3://{bucket}/bronze/003_bronze_tab_cliente/part-{part + 1:05d}.parquet"
...
def main():
    ...
    bronze_prefix = "bronze/003_bronze_tab_cliente"

    clear_minio_prefix(fs, minio_cfg.bucket, bronze_prefix)

    spec = TableSpec(schema="public", name="tab_cliente", chunk_rows=50_000, itersize=10_000)
    ...
    s3_path = f"s3://{minio_cfg.bucket}/bronze/003_bronze_tab_cliente/part-00001.parquet"
    local_folder = "/home/Projetos/Docker/Orquestradores/airflow/data/Sost/Datalake/Bronze/003_bronze_tab_cliente"

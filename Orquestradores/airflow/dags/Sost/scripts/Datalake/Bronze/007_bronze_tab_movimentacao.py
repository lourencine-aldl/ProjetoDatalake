#!/usr/bin/env python3
"""
Bronze Layer ETL: Extração TAB_MOVIMENTACAO PostgreSQL → MinIO (Polars)
"""
...
logger = logging.getLogger("bronze_007_tab_movimentacao")
...
def make_s3_path(bucket: str, schema: str, table: str, ingestion_dt: datetime, part: int) -> str:
    return f"s3://{bucket}/bronze/007_bronze_tab_movimentacao/part-{part + 1:05d}.parquet"
...
def main():
    ...
    bronze_prefix = "bronze/007_bronze_tab_movimentacao"

    clear_minio_prefix(fs, minio_cfg.bucket, bronze_prefix)

    spec = TableSpec(schema="public", name="tab_movimentacao", chunk_rows=50_000, itersize=10_000)
    ...
    s3_path = f"s3://{minio_cfg.bucket}/bronze/007_bronze_tab_movimentacao/part-00001.parquet"
    local_folder = "/home/Projetos/Docker/Orquestradores/airflow/data/Sost/Datalake/Bronze/007_bronze_tab_movimentacao"
